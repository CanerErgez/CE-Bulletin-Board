...
/* Güncelleme İçin Kodlar */
	...
	$sql=mysqli_query($db,"DELETE FROM forumlar WHERE id='$forum_id'");

	while(true) {
		$konular_al=mysqli_query($db,"SELECT * FROM konular WHERE f_id='$forum_id'");
			while ($konu_mfa=mysqli_fetch_array($konular_al,MYSQLI_ASSOC)) {
				$konu_id=$konu_mfa['id'];
					$sql_new=mysqli_query($db,"DELETE FROM cevaplar WHERE konu_id='$konu_id'");
					if(!$sql_new){
						die;
						echo "Cevaplar Silenemedi";
					}
			}
		$sql_new_2=mysqli_query($db,"DELETE FROM konular WHERE f_id='$forum_id'");
			if(!$sql_new_2){
				die;
				echo "Konular Silenemedi";
			}
		break;
	}
	...
	if($sql) {
		echo '<div class="container theme-showcase" role="main">
                    <div class="alert alert-success">
                        <strong>Tebrikler !</strong> Forum Başarıyla Silindi .
      ...
      <div class="row">
        <div class="col-sm-3 col-md-2 sidebar">
          ...
          <ul class="nav nav-sidebar">
            ...
            <li class="active"><a href="del_forum.php">Forum Sil</a></li>
          </ul>
		  <ul class="nav nav-sidebar">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h1 class="page-header">Forum Sil</h1>
		<?php
		while($forumlar_info=mysqli_fetch_array($sql,MYSQLI_ASSOC)) {
		?>

		  <form class="form-signin" role="form" action="del_forum.php" method="GET">
			<input type="hidden" name="forum_id" value="<?php echo $forumlar_info['id']; ?>" />
			<input type="text" class="form-control" value="<?php echo $forumlar_info['forum_ad']; ?>" /><br />
			<input type="text" class="form-control" value="<?php echo $forumlar_info['forum_aciklama']; ?>"  /><br />
			<center>
