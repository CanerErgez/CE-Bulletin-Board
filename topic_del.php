...
if(isset($_POST['don_id'])) {
	...
	$forum_id_cek=mysqli_query($db,"SELECT f_id FROM konular WHERE id='$konu_for_id'");
	$forum_id_cek_mfa=mysqli_fetch_array($forum_id_cek,MYSQLI_ASSOC);
	$forum_id=$forum_id_cek_mfa['f_id'];


	$sql=mysqli_query($db,"DELETE FROM konular WHERE id='$konu_for_id'");
	$sql_cevap=mysqli_query($db,"DELETE FROM cevaplar WHERE konu_id='$konu_for_id'");

	if($sql && $sql_cevap) {
	echo '<div class="container theme-showcase" role="main">
                    <div class="alert alert-success">
                        <strong>Tebrikler !</strong> Konu Başarıyla Silindi .
						<meta http-equiv="refresh" content="1;URL=viewforum.php?forum='.$forum_id.'">
                    </div>
               </div>';
...
<html lang="tr">
  <head>
    ...
    <meta name="description" content="Konu Silme Sayfası">
    <link rel="shortcut icon" href="assets/ico/favicon.ico">

    <title>Konu Sil</title>
...
?>
	...
	<div class="container">
		<form class="form-signin" role="form" action="topic_del.php" method="POST">
			<input name="don_id" type="hidden" value="<?php echo $konu_id; ?>">
		  <p><strong><?php echo $konu_baslik; ?></strong> Başlıklı Konuyu Sİlmek Istediğinize Emin Misiniz ?</p>
			<center>
			<button class="btn btn-lg btn-danger" type="submit">Sil</button>
			</center>
		</form>
	</div>
