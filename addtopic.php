...
if(isset($_POST['konu_baslik']) && isset($_POST['konu_aciklama']) && isset($_POST['for_id'])) {
	...
	$konu_for_id=htmlspecialchars(mysqli_real_escape_string($db,trim($_POST['for_id'])));
	$konu_gonderen=$_SESSION['kullanici'];

	$sql = mysqli_query($db,"INSERT INTO konular (f_id,gonderen,isim,aciklama,songonderen) VALUES ('$konu_for_id','$konu_gonderen','$konu_baslik','$konu_aciklama','$konu_gonderen')");

	if($sql){
		echo '<div class="container theme-showcase" role="main">
                    <div class="alert alert-success">
                        <strong>Tebrikler !</strong> Konu Başarıyla Oluşturuldu .
						<meta http-equiv="refresh" content="1;URL=viewforum.php?forum='.$konu_for_id.'">
                    </div>
               </div>';
	} else {
		echo'<div class="container theme-showcase" role="main">
    ...
    $forum_id=htmlspecialchars(mysqli_real_escape_string($db,trim($_GET['f_id'])));

	$sql_kontrol=mysqli_query($db,"SELECT * FROM forumlar WHERE id='$forum_id'");

	if(!($sql_kontrol_var = mysqli_fetch_array($sql_kontrol,MYSQLI_ASSOC))) {
	echo 'Geçersiz Bir Forum Adresi Girdiniz .';
	die;
	}
} else {
...
<html lang="tr">
  <head>
    ...
    <meta name="description" content="Konu Aç Sayfası">
...
?>
	...
	<div class="container">
		<form class="form-signin" role="form" action="addtopic.php" method="POST">
			<input name="for_id" type="hidden" value="<?php echo $forum_id; ?>">
			<input name="konu_baslik" type="text" class="form-control" placeholder="Başlığınız" required><br/>
			<textarea class="form-control" rows="10" name="konu_aciklama"></textarea><br/>
			<center>
